...
It is deliberately **not** a clipboard *history* tool. Nothing is captured
...
moves. And because "which letter was slot 5 again?" is a real problem — **hold
`Ctrl` and every Copy button turns into its own shortcut letter.** Let go and
they turn back. The app teaches you its shortcuts while you use it.
...
setting, close the window, reboot — it's all still there.
...
- **Windows** (Windows 10 or 11; the dark title bar needs Windows 10 1809+, and
  degrades gracefully on older builds)
...
Download `OctoCopy.exe` and double-click it. That's the entire install — no
installer, no registry keys, no dependencies. Settings go in
`%APPDATA%\OctoCopy\`, which is the only thing it writes outside itself.
...
### `Program.cs` — the entry point and the single-instance guard
...
hotkey — and Windows gives `Ctrl + Alt + C` to exactly one process. The second
one silently wouldn't work, which is a maddening bug to diagnose.
...
`snippetPanel.ClientSize.Width` — which already excludes the scrollbar. Edges
stay clean whether the bar is there or not.
...
peeks at the first character — `[` means the old format — and reads it the old
...
P/Invoke: `RegisterHotKey` from `user32.dll` claims the combination
system-wide, and the form overrides `WndProc` — the raw Windows message handler —
to watch for `WM_HOTKEY` (message `0x0312`). When it arrives, the window shows,
activates, and comes to the front. `UnregisterHotKey` releases the claim on
close, so the combination isn't held hostage after the app exits.
...
well — otherwise alt-tabbing away while holding Ctrl would leave the buttons
stuck showing letters forever, since the key-up event goes to whatever window you
switched to.

#### The system tray

A `NotifyIcon` with a `ContextMenuStrip`. The menu is rebuilt from scratch every
time it opens, in the `Opening` event, so it always reflects the current snippets
— text is stripped of line breaks and truncated to 40 characters, and each item
gets a closure capturing its own text to copy.

Left-clicking a tray icon doesn't normally open its context menu; only
right-click does. There's no public API for it, so the code uses **reflection** to
find and call `NotifyIcon`'s private `ShowContextMenu` method. It's a hack, and
it's guarded with `?.Invoke` so a future .NET change that renames the method
...
- `DWMWA_USE_IMMERSIVE_DARK_MODE` (attribute 20), plus attribute 19 as a fallback
  — the constant changed between Windows 10 builds, so it sets both and lets the
  one that isn't recognised fail harmlessly.
...
control. It has one specific carve-out: it never recolors a button that's
currently `LightGreen`, so toggling dark mode mid-flash doesn't leave a button
stuck green.
...
and nullable window coordinates. `WindowLocationX/Y` are nullable specifically so
"never saved a position" is distinguishable from "saved position 0,0".
...
| `OctoCopy.exe` | Prebuilt release binary. |
| `OctoCopy_1.0.exe` | An older prebuilt release. |

---

## Before you publish this repo

No credentials or personal data here — but there is build output committed that
shouldn't be.

**`bin/` and `obj/` are in the repo.** These are compiler artifacts: `.exe`,
`.pdb` debug symbols, `.cache` files, and absolute paths from the machine that
built them (`App.csproj.FileListAbsolute.txt` literally contains your folder
layout). They're regenerated by every build and never belong in version control.

**The `.exe` files at the root are a judgement call.** GitHub's convention is to
attach binaries to a *Release* rather than commit them, because every future
version permanently bloats the repo's history. Consider deleting both and
uploading `OctoCopy.exe` as a v1.0 release asset — the download link stays just
as easy for users.
...
Since `bin/` and `obj/` are already committed, a `.gitignore` alone won't remove
them — run `git rm -r --cached bin obj` after adding it.
...
- **The tray left-click uses reflection.** It calls a private .NET method. A
  future framework change could break it — safely, but it would break.
